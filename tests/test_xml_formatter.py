...
class TestXmlFormatter(unittest.TestCase):
    ...
    def test_format_tree_preorder_traversal(self):
        ...
        root.session_xml = "<session><prompt>Root</prompt><submit>Root result</submit></session>"
        child1.session_xml = "<session><prompt>Child 1</prompt><submit>Child 1 result</submit></session>"
        child2.session_xml = "<session><prompt>Child 2</prompt><submit>Child 2 result</submit></session>"
    ...
    def test_format_tree_pretty_printing(self):
        ...
        root.session_xml = "<session><prompt>Test</prompt><submit>Result</submit></session>"
        
        result = self.formatter.format_tree_xml(root)
        
        # Should have proper indentation
        lines = result.split('\n')
        self.assertTrue(any('  <session>' in line for line in lines))  # Indented session
        self.assertTrue(any('    <id>0</id>' in line for line in lines))  # Deeper indentation
    ...
    def test_format_tree_with_special_characters(self):
        ...
        root = TreeNode(session_id=0, prompt="Test with <special> & \"characters\"", depth=0)
        root.session_xml = """<session>
    ...
    def test_format_tree_large_tree(self):
        ...
        for i in range(10):
            ...
            for j in range(2):
                grandchild = TreeNode(session_id=10+(i*2)+j+1, prompt=f"Grandchild {i}-{j}", depth=2)
