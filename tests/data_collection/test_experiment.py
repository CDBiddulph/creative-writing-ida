...
<sessions>
  ...
  <session>
    ...
    def test_creates_new_experiment_directory_structure(
    ...
    def test_resumes_existing_experiment_from_correct_iteration(
        self, test_config, mock_tree_runner
    ):
        ...
        iter0_used = json.loads(
            (exp_path / "iteration_0" / "used_prompts.json").read_text()
        )
        iter1_used = json.loads(
            (exp_path / "iteration_1" / "used_prompts.json").read_text()
        )
    ...
    def test_experiment_handles_parent_generation_gracefully(
        self, test_config, mock_tree_runner
    ):
        """Test that experiment fails gracefully when parent generation is needed but not implemented."""
        config, tmpdir = test_config

        # Enable parent generation
        config.parent_examples_per_iteration = 1
        config.max_parent_examples = 5
        config.max_iterations = 1

        experiment = Experiment(config, base_dir=Path(tmpdir))

        # Should raise RuntimeError about parent generation not being implemented
        with pytest.raises(
            RuntimeError, match="Parent example generation requires a web UI"
        ):
            experiment.run()
    ...
    def test_handles_insufficient_prompts_gracefully(
        self, test_config, mock_tree_runner
    ):
        ...
        with pytest.raises(RuntimeError, match="Prompt sampling failed"):
            experiment.run()
    ...
    def test_sample_sessions_have_correct_filenames(
        ...
        sample_sessions = list(
            (exp_path / "iteration_0" / "sample-sessions").glob("*.xml")
        )
    ...
    def test_leaf_sessions_have_node_id_in_filename(
        self, test_config, mock_tree_runner
    ):
    ...
    def test_used_prompts_accumulate_correctly(self, test_config, mock_tree_runner):
        ...
        iter2_used = json.loads(
            (exp_path / "iteration_2" / "used_prompts.json").read_text()
        )
