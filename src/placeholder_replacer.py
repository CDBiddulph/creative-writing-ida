...
class PlaceholderReplacer:
    ...
    def build_replacement_map(self, session: Session) -> Dict[str, str]:
        ...
        # Extract prompt text
        prompt_events = [
            event for event in session.events if isinstance(event, PromptEvent)
        ]
        if prompt_events:
        ...
        response_events = [
            event for event in session.events if isinstance(event, ResponseEvent)
        ]
    ...
    def replace_placeholders(self, text: str, replacement_map: Dict[str, str]) -> str:
        ...
        placeholders_to_replace = [
            p for p in placeholders_in_text if p in replacement_map
        ]
