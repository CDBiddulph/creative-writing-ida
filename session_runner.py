...
from api_interface import get_api_interface, APIInterface
...
class SessionRunner:
    ...
    def __init__(
        ...
        max_tokens: int,
        temperature: float,
        parent_readme_content: str,
        leaf_readme_content: str,
        parent_examples_xml: str = "",
        leaf_examples_xml: str = ""
    ):
        self.model = resolve_model_name(model)
        self.max_tokens = max_tokens
        self.temperature = temperature
        self.parent_readme_content = parent_readme_content
        self.leaf_readme_content = leaf_readme_content
        self.parent_examples_xml = parent_examples_xml
        self.leaf_examples_xml = leaf_examples_xml
        
        # Determine API mode based on model
        if self.model.startswith("as-hackathon"):
            self.api_mode = "claude-completions"
        else:
            self.api_mode = "claude-base"
    ...
    def _execute_parent_session(self, node: TreeNode) -> None:
        ...
        """
        # Get API interface
        api = get_api_interface(self.api_mode, readme_content=self.parent_readme_content)
        
        # Call API with parent configuration
        response_text = api.call(
            prompt=node.session.prompt,
            model=self.model,
            max_tokens=self.max_tokens,
            temperature=self.temperature,
            examples_xml=self.parent_examples_xml
        )
    ...
    def _execute_leaf_session(self, session: Session) -> None:
        ...
        """
        # Get API interface  
        api = get_api_interface(self.api_mode, readme_content=self.leaf_readme_content)
        ...
        response_text = api.call(
            ...
            max_tokens=self.max_tokens,
            temperature=self.temperature,
            examples_xml=self.leaf_examples_xml
        )
    ...
    def _split_response_sections(self, response: str) -> List[tuple[str, str]]:
        ...
        if not sections:
            # Simple fallback: treat entire response as submit
            sections.append(('submit', response.strip()))
